defmodule GenServerAsync.RegisterUserTest do
  ...
  test "expect add use to state", %{pid: pid, valid_user: user} do
    task = Task.async fn ->
      assert {:ok, %User{name: "valid_user", state: :registered}} = GenServerAsync.call_async(pid, {:register, user})
      assert %User{name: "valid_user", state: :registered} = GenServerAsync.call(pid, {:get, user.name})
    end
    Process.sleep(10)
  ...
  test "expect no add invalid user to state", %{pid: pid, invalid_user: user}  do
    ...
    Process.sleep(10)
    assert assert %User{name: "invalid_user", state: :invalid} = GenServerAsync.call(pid, {:get, user.name})
    Task.await(task)
    assert :not_found == GenServerAsync.call(pid, {:get, user.name})
  end
